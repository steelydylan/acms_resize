...
class ACMS_User_Hook
{
    ...
    public function beforePostFire($thisModule)
    {
        ...
        foreach($_POST as $key => $value){
            if(preg_match("/image_replace/",$key,$match)){
                $binary = base64_decode($value); 
                $file   = uniqueString().'.png';        // 保存ファイル名
                $destDir = ACMS_POST::archivesDir();
                $dest   = ARCHIVES_DIR.$destDir.$file;  // 保存先
                file_put_contents($dest,$binary);
                $old = str_replace("image_replace","image_old",$key);
                $dest = str_replace("archives/", "", $dest);
                $_POST[$old] = $dest;
            }
        }
    }
